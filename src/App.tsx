...
import { addServiceEntry, addVehicle, archiveVehicle, importServiceEntries, loadEntries, loadVehicles, updateVehicle, type ImportedServiceEntryValues } from "./data";
...
function formatDate(value?: Timestamp) { return value ? value.toDate().toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" }) : "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"; }
...
export default function App() {
  ...
  const selectedVehicle = view.name === "vehicle" || view.name === "addEntry" || view.name === "editVehicle"
      ...
      <div className="app-main">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <button className="icon-button" aria-label="Notifications">ÃƒÂ¢Ã¢â€žÂ¢Ã‚Â¢</button>
            <button className="avatar" onClick={() => void handleAvatarClick()} aria-label={demo ? "Exit local preview" : "Sign out"} title={demo ? "Exit local preview" : "Sign out"}>{demo ? "CM" : (user?.displayName ?? user?.email ?? "L").slice(0, 2).toUpperCase()}</button>
          </div>
        </header>
        ...
        {loading ? <LoadingCards /> : (
          <>
            ...
            {view.name === "editVehicle" && selectedVehicle && <VehicleDetailsForm vehicle={selectedVehicle} onCancel={() => navigate({ name: "vehicle", vehicleId: selectedVehicle.id })} onSave={(values) => saveVehicleDetails(selectedVehicle.id, values)} />}
...
function Sidebar({ view, navigate, demo, user }: { view: View; navigate: (view: View) => void; demo: boolean; user: User | null }) {
  return <aside className="sidebar"><button className="wordmark" onClick={() => navigate({ name: "garage" })}><span><b>Ledger</b></span></button><nav aria-label="Main navigation"><SidebarLink active={view.name === "garage"} icon="ÃƒÂ¢Ã…â€™Ã¢â‚¬Å¡" label="My Garage" onClick={() => navigate({ name: "garage" })} /><SidebarLink icon="ÃƒÂ¢Ã¢â‚¬â€Ã‚Â·" label="Reminders" /><SidebarLink icon="ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â±" label="Documents" /><SidebarLink icon="ÃƒÂ¢Ã…â€™Ã‚Â" label="Reports" /><SidebarLink icon="ÃƒÂ¢Ã…Â¡Ã¢â€žÂ¢" label="Settings" /></nav><div className="sidebar-foot"><p>{demo ? "Previewing local sample data" : user?.email}</p>{!demo && <button onClick={() => void signOut(auth)}>Sign out</button>}</div></aside>;
}
...
function VehiclePage({ vehicle, entries, navigate, onEdit, onImport, onArchive }: { vehicle: Vehicle; entries: ServiceEntry[]; navigate: (view: View) => void; onEdit: () => void; onImport: () => void; onArchive: () => void }) {
  ...
  return <main className="page vehicle-page"><button className="back-link" onClick={() => navigate({ name: "garage" })}>ÃƒÂ¢Ã¢â‚¬Â Ã‚Â My Garage</button><section className="vehicle-hero"><div><p className="kicker">{vehicle.nickname || "Vehicle record"}</p><h1>{vehicle.year} {vehicle.make}<br /><em>{vehicle.model}</em><span>.</span></h1><p>{vehicle.trim || "Details can be added at any time"}</p></div><div className="hero-mileage"><span>Latest recorded mileage</span><strong>{vehicle.latestMileage?.toLocaleString() ?? "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}</strong><small>{vehicle.latestMileage ? `miles Ãƒâ€šÃ‚Â· ${formatDate(vehicle.latestMileageDate)}` : "Add mileage with your next entry"}</small></div><div className="hero-actions"><button className="primary-button" onClick={() => navigate({ name: "addEntry", vehicleId: vehicle.id })}>ÃƒÂ¯Ã‚Â¼Ã¢â‚¬Â¹ New entry</button><button className="quiet-button" onClick={onEdit}>Edit details</button><button className="quiet-button" onClick={onImport}>Import CSV</button><button className="quiet-button" onClick={onArchive}>Archive vehicle</button></div></section><div className="vehicle-tabs" role="navigation" aria-label="Vehicle sections"><button className="active">Overview</button><button>Timeline</button><button>Documents</button><button>Costs</button><button onClick={onEdit}>Details</button></div><section className="overview-grid"><div className="timeline-panel"><div className="panel-heading"><div><p className="kicker">Vehicle timeline</p><h2>Recent history</h2></div>{sorted.length > 0 && <span>{sorted.length} {sorted.length === 1 ? "entry" : "entries"}</span>}</div>{sorted.length === 0 ? <div className="empty-timeline"><h3>Record what happened</h3><p>Services, repairs, journeys, and observations will appear here.</p><button className="primary-button" onClick={() => navigate({ name: "addEntry", vehicleId: vehicle.id })}>Create first entry</button></div> : <div className="timeline">{sorted.map((entry) => <TimelineItem entry={entry} key={entry.id} />)}</div>}</div><aside className="overview-side"><div className="side-card reminder-card"><p className="kicker">Next up</p><h3>Nothing is currently due</h3><p>Add a reminder from a future service entry.</p><button>ÃƒÂ¯Ã‚Â¼Ã¢â‚¬Â¹ Add reminder</button></div><div className="side-card"><p className="kicker">Recorded costs</p><strong>{formatCost(total) ?? "$0.00"}</strong><p>Across {sorted.length} recorded {sorted.length === 1 ? "entry" : "entries"}</p></div><div className="side-card identity-card"><p className="kicker">Vehicle details</p><dl><div><dt>Year</dt><dd>{vehicle.year}</dd></div><div><dt>Make</dt><dd>{vehicle.make}</dd></div><div><dt>Model</dt><dd>{vehicle.model}</dd></div>{vehicle.vin && <div><dt>VIN</dt><dd>{vehicle.vin.slice(-6).padStart(vehicle.vin.length, "ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¢")}</dd></div>}</dl></div></aside></section></main>;
}
...
function CsvImportForm({ vehicle, onCancel, onSave }: { vehicle: Vehicle; onCancel: () => void; onSave: (rows: ImportedServiceEntryValues[]) => Promise<void> }) {
  ...
  return <main className="form-page"><button className="back-link" onClick={onCancel}>â† {vehicleName(vehicle)}</button><div className="form-layout"><div className="form-intro"><p className="kicker">Import history</p><h1>Bring in<br />the record<span>.</span></h1><p>Review every row before it becomes part of this vehicle&rsquo;s timeline.</p></div><section className="ledger-form"><fieldset><legend>Maintenance CSV</legend><label>Choose CSV file<input type="file" accept=".csv,text/csv" onChange={(event) => void chooseFile(event)} /></label><p className="field-help">Supported columns: Date, Odom., Shop, Total Cost, Service, and Estimated Dealer Cost.</p></fieldset>{rows.length > 0 && <><div className="import-summary"><strong>{rows.length} entries ready</strong><span>{formatCost(rows.reduce((total, row) => total + (row.totalCostCents ?? 0), 0))} recorded cost</span><span>{formatCost(rows.reduce((total, row) => total + (row.estimatedShopCostCents ?? 0), 0))} estimated shop cost</span></div><div className="import-preview" aria-label="CSV import preview">{rows.slice(0, 8).map((row, index) => <div key={`${row.serviceDate}-${index}`}><span>{row.serviceDate}</span><b>{row.description}</b><small>{row.mileage?.toLocaleString()} mi Â· {formatCost(row.totalCostCents) ?? "No cost"} Â· Est. {formatCost(row.estimatedShopCostCents) ?? "â€”"}</small></div>)}{rows.length > 8 && <p className="field-help">Plus {rows.length - 8} more entries.</p>}</div></>}{error && <p className="form-error" role="alert">{error}</p>}<div className="form-actions"><button type="button" className="quiet-button" onClick={onCancel}>Cancel</button><button type="button" className="primary-button" disabled={!rows.length || saving} onClick={() => void submit()}>{saving ? "Importingâ€¦" : `Import ${rows.length || ""} entries â†’`}</button></div></section></div></main>;
}
...
function EntryForm({ vehicle, onCancel, onSave }: { vehicle: Vehicle; onCancel: () => void; onSave: (values: EntryFormValues) => Promise<void> }) {
  ...
  return <main className="entry-page"><button className="back-link" onClick={onCancel}>ÃƒÂ¢Ã¢â‚¬Â Ã‚Â {vehicleName(vehicle)}</button><div className="entry-heading"><p className="kicker">New entry Ãƒâ€šÃ‚Â· {vehicle.nickname || vehicleName(vehicle)}</p><h1>What happened<span>?</span></h1><p>Write it the way you&rsquo;d tell a friend. The details can come later.</p></div><form className="entry-form" onSubmit={submit}><label className="description-field"><span className="sr-only">What happened?</span><textarea name="description" required autoFocus placeholder="Replaced rear brake pads and rotorsÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦" rows={4} /></label><div className="entry-fields"><label>Date<input type="date" name="date" required defaultValue={today} /></label><label>Mileage <small>Recommended</small><input type="number" name="mileage" min="0" placeholder={vehicle.latestMileage?.toLocaleString() || "Current mileage"} /></label><label>Cost <small>Optional</small><div className="money-field"><span>$</span><input type="number" name="cost" min="0" step="0.01" placeholder="0.00" /></div></label><label>Performed by <small>Optional</small><select name="provider" defaultValue="diy"><option value="diy">Me</option><option value="shop">Shop</option><option value="dealer">Dealer</option><option value="other">Other</option></select></label></div><button type="button" className="details-toggle" onClick={() => setDetails(!details)}>{details ? "ÃƒÂ¢Ã‹â€ Ã¢â‚¬â„¢ Hide details" : "+ Add details"}</button>{details && <div className="details-note"><p>Categories, parts, reminders, warranties, and private notes are coming next. The basic record is saved first.</p></div>}<button type="button" className="attachment-button"><span>ÃƒÂ¢Ã¢â‚¬â€œÃ‚Â±</span><b>Add receipt or photos</b><small>Available in Sprint 1</small></button>{error && <p className="form-error" role="alert">{error}</p>}<div className="entry-save"><p>Saved entries remain useful even if uploads or AI are unavailable.</p><button className="primary-button" disabled={saving}>{saving ? "Saving entryÃƒÂ¢Ã¢â€šÂ¬Ã‚Â¦" : "Save entry ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢"}</button></div></form></main>;
}
