...
import { addMaintenanceItem, addServiceEntry, deleteMaintenanceItem, loadMaintenanceItems, addVehicle, archiveVehicle, importServiceEntries, loadEntries, loadVehicles, recordMileage, updateServiceEntry, updateVehicle, uploadEntryPhoto, uploadVehiclePhoto, filterServiceEntries, updateMaintenanceItem, addVehicleSpecification, deleteVehicleSpecification, loadVehicleSpecifications, updateVehicleSpecification, type ImportedServiceEntryValues } from "./data";
import { parseMaintenanceCsv } from "./csv";
import { maintenanceDue, normalizeMoneyToCents, shouldAdvanceMileage } from "./domain";
import { decodeVin, isValidVin, normalizeVin } from "./vin";
import type { MaintenanceItem, ServiceEntry, Vehicle, VehicleSpecification, View } from "./types";
...
function VehiclePage({ vehicle, entries, navigate, onEdit, onImport, onArchive }: { vehicle: Vehicle; entries: ServiceEntry[]; navigate: (view: View) => void; onEdit: () => void; onImport: () => void; onArchive: () => void }) {
  ...
  return <main className="page vehicle-page"><button className="back-link" onClick={() => navigate({ name: "garage" })}>← My Garage</button><section className="vehicle-hero"><div><p className="kicker">{vehicle.nickname || "Vehicle record"}</p><h1>{vehicle.year} {vehicle.make}<br /><em>{vehicle.model}</em><span>.</span></h1><p>{vehicle.trim}</p></div><div className="hero-mileage"><span>Latest recorded mileage</span><strong>{vehicle.latestMileage?.toLocaleString() ?? "—"}</strong><small>{vehicle.latestMileage ? formatDate(vehicle.latestMileageDate) : "Add mileage with your next entry"}</small></div><div className="hero-actions"><button className="primary-button" onClick={() => navigate({ name: "addEntry", vehicleId: vehicle.id })}>＋ New entry</button><button className="quiet-button" onClick={onEdit}>Edit details</button><button className="quiet-button" onClick={onImport}>Import CSV</button><button className="quiet-button" onClick={onArchive}>Archive vehicle</button></div></section><section className="vehicle-summary"><div className="side-card reminder-card"><p className="kicker">Due & upcoming</p><h3>Maintenance stays explicit</h3><p>Plan items below use their recorded completion mileage and date.</p></div><div className="side-card"><p className="kicker">Recorded costs</p><strong>{formatCost(total) ?? "$0.00"}</strong><p>Across {sorted.length} recorded {sorted.length === 1 ? "entry" : "entries"}</p></div></section><MaintenancePanel vehicle={vehicle} entries={sorted} /><VehicleReferencePanel vehicle={vehicle} /><section className="timeline-panel service-history"><div className="panel-heading"><div><p className="kicker">Service history</p><h2>The record</h2></div>{sorted.length > 0 && <span>{sorted.length} {sorted.length === 1 ? "entry" : "entries"}</span>}</div><label className="history-search"><span className="sr-only">Search service history</span><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search service history…" /></label>{sorted.length === 0 ? <div className="empty-timeline"><h3>Record what happened</h3><p>Services, repairs, and observations will appear here.</p><button className="primary-button" onClick={() => navigate({ name: "addEntry", vehicleId: vehicle.id })}>Create first entry</button></div> : matches.length === 0 ? <p className="history-empty">No service entries match “{debouncedSearch}”.</p> : <div className="timeline">{matches.map((entry) => <TimelineItem entry={entry} key={entry.id} onEdit={() => navigate({ name: "editEntry", vehicleId: vehicle.id, entryId: entry.id })} />)}</div>}</section></main>;
}

function MaintenancePanel({ vehicle, entries }: { vehicle: Vehicle; entries: ServiceEntry[] }) {
  const isDemo = vehicle.ownerUserId === "demo"; const [items, setItems] = useState<MaintenanceItem[]>([]); const [name, setName] = useState(""); const [interval, setInterval] = useState(""); const [unit, setUnit] = useState<"miles" | "months">("miles"); const [editing, setEditing] = useState<MaintenanceItem>();
  useEffect(() => { if (!isDemo) void loadMaintenanceItems(vehicle.ownerUserId, vehicle.id).then(setItems); }, [vehicle.id, vehicle.ownerUserId, isDemo]);
