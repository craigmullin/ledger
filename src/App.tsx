import { type ChangeEvent, type Dispatch, FormEvent, type SetStateAction, useEffect, useMemo, useState } from "react";
import {
...
import { addMaintenanceItem, addServiceEntry, deleteMaintenanceItem, loadMaintenanceItems, loadAllMaintenanceItems, loadAttachments, addVehicle, archiveVehicle, importServiceEntries, loadEntries, loadVehicles, recordMileage, updateServiceEntry, updateVehicle, uploadEntryPhoto, uploadVehiclePhoto, filterServiceEntries, updateMaintenanceItem, addVehicleSpecification, deleteVehicleSpecification, loadVehicleSpecifications, updateVehicleSpecification, type ImportedServiceEntryValues } from "./data";
import { parseMaintenanceCsv } from "./csv";
import { compareMaintenanceDue, maintenanceDue, normalizeMoneyToCents, shouldAdvanceMileage } from "./domain";
import { decodeVin, isValidVin, normalizeVin } from "./vin";
import type { Attachment, MaintenanceItem, ServiceEntry, Vehicle, VehicleSpecification, View } from "./types";
...
const demoMaintenanceItems: MaintenanceItem[] = [
  { id: "m1", ownerUserId: "demo", vehicleId: "demo-crv", name: "Oil change", priority: "normal", enabled: true, intervalMiles: 5000, lastDoneMileage: 114208, schemaVersion: 1 },
  { id: "m2", ownerUserId: "demo", vehicleId: "demo-crv", name: "Tire rotation", priority: "normal", enabled: true, intervalMiles: 7500, lastDoneMileage: 112000, schemaVersion: 1 },
];
...
function maintenanceValues(item: MaintenanceItem) { return { intervalMiles: item.intervalMiles, intervalMonths: item.intervalMonths, lastDoneMileage: item.lastDoneMileage, lastDoneDate: item.lastDoneDate?.toDate() }; }
function nextMaintenance(items: MaintenanceItem[], vehicle: Vehicle) { return items.filter((item) => item.enabled).sort((a, b) => compareMaintenanceDue(maintenanceValues(a), maintenanceValues(b), vehicle.latestMileage))[0]; }
function dueText(item: MaintenanceItem | undefined, vehicle: Vehicle) { if (!item) return "Nothing scheduled"; const due = maintenanceDue(maintenanceValues(item), vehicle.latestMileage); const detail = [due.mileageDelta != null && (due.mileageDelta < 0 ? `${Math.abs(due.mileageDelta).toLocaleString()} mi overdue` : `in ${due.mileageDelta.toLocaleString()} mi`), due.dateDelta != null && (due.dateDelta < 0 ? `${Math.abs(due.dateDelta)} days overdue` : `in ${due.dateDelta} days`)].filter(Boolean)[0]; return `${item.name}${detail ? ` · ${detail}` : " · Not scheduled"}`; }
...
export default function App() {
  ...
  const [maintenanceItems, setMaintenanceItems] = useState<MaintenanceItem[]>(demo ? demoMaintenanceItems : []);
  ...
  useEffect(() => {
    if (!ownerId || demo) return;
    loadAllMaintenanceItems(ownerId).then(setMaintenanceItems).catch(() => setError("We couldn't load maintenance plans."));
  }, [ownerId, demo]);
  ...
  if (!ownerId) return showAuth
    ...
    : <PublicLanding onDemo={() => { setDemo(true); setVehicles(demoVehicles); setEntries(demoEntries); setView({ name: "garage" }); }} onSignIn={() => setShowAuth(true)} />;
  ...
  async function saveEditedEntry(entry: ServiceEntry, values: EntryFormValues) {
    ...
    if (!ownerId) return;
    if (photoFiles?.length) await Promise.all(photoFiles.map((file) => uploadEntryPhoto(ownerId, entry.vehicleId, entry.id, file)));
      ...
      <div className="app-main">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <button className="icon-button notification-count" aria-label="Notifications: 0">0</button>
        ...
        {loading ? <LoadingCards /> : (
          <>
            {view.name === "garage" && <Garage vehicles={vehicles} maintenanceItems={maintenanceItems} navigate={navigate} onSaveMileage={saveMileage} />}
            {view.name === "documents" && <DocumentsPage ownerUserId={ownerId} vehicles={vehicles} entries={entries} navigate={navigate} demo={demo} />}
...
function Garage({ vehicles, maintenanceItems, navigate, onSaveMileage }: { vehicles: Vehicle[]; maintenanceItems: MaintenanceItem[]; navigate: (view: View) => void; onSaveMileage: (vehicleId: string, mileage: number) => Promise<void> }) {
  ...
  return <main className="page"><div className="page-heading"><div><h1>My vehicles<span>.</span></h1></div><button className="primary-button" onClick={() => navigate({ name: "addVehicle" })}><span>+</span> Add vehicle</button></div>{vehicles.length > 0 && <section className="quick-mileage"><div className="quick-mileage-heading"><p className="kicker">Quick mileage</p><strong>Record today&rsquo;s odometer</strong></div><div className="quick-mileage-controls"><select value={vehicleId} onChange={(event) => setVehicleId(event.target.value)} aria-label="Vehicle">{vehicles.map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicleName(vehicle)}</option>)}</select><label className="mileage-input"><span>Odometer</span><input type="text" inputMode="numeric" pattern="[0-9]*" value={mileage} onChange={(event) => setMileage(event.target.value.replace(/\D/g, ""))} placeholder="Mileage" aria-label="Current mileage" /></label><button className="save-mileage-button" type="button" disabled={saving || !mileage} onClick={() => void save()}>{saving ? "Saving…" : "Save mileage"}</button></div></section>}{vehicles.length === 0 ? <EmptyGarage onAdd={() => navigate({ name: "addVehicle" })} /> : <div className="garage-grid">{vehicles.map((vehicle, index) => { const closest = nextMaintenance(maintenanceItems.filter((item) => item.vehicleId === vehicle.id), vehicle); return <button className="vehicle-card" key={vehicle.id} onClick={() => navigate({ name: "vehicle", vehicleId: vehicle.id })}><div className={`vehicle-art art-${index % 3}`} style={{ backgroundColor: vehicle.color || undefined }}>{vehicle.heroImageUrl ? <img className="vehicle-art-photo" src={vehicle.heroImageUrl} alt="" /> : <div className="vehicle-placeholder" aria-hidden="true" />}<span className="volume">{String(index + 1).padStart(2, "0")}</span></div><div className="vehicle-card-body"><div><p className="card-label">{vehicle.nickname || "In your garage"}</p><h2>{vehicleName(vehicle)}</h2><p>{vehicle.licensePlate || "Plate not recorded"}</p></div><div className="vehicle-facts"><div><span>Latest mileage</span><b>{formatMileage(vehicle.latestMileage)}</b></div><div><span>Next maintenance</span><b>{dueText(closest, vehicle)}</b></div></div></div></button>; })}</div>}</main>;
}

function EmptyGarage({ onAdd }: { onAdd: () => void }) { return <section className="empty-state"><div className="empty-art"><div className="garage-mark" aria-hidden="true" /></div><p className="kicker">A clean start</p><h2>Add your first vehicle</h2><p>Ledger begins with the machine. Add the basics now—you can fill in the details whenever you like.</p><button className="primary-button" onClick={onAdd}>Add your first vehicle</button></section>; }
...
function VehiclePage({ vehicle, entries, navigate, onEdit, onImport, onArchive }: { vehicle: Vehicle; entries: ServiceEntry[]; navigate: (view: View) => void; onEdit: () => void; onImport: () => void; onArchive: () => void }) {
  ...
  const isDemo = vehicle.ownerUserId === "demo";
  const [maintenanceItems, setMaintenanceItems] = useState<MaintenanceItem[]>(isDemo ? demoMaintenanceItems.filter((item) => item.vehicleId === vehicle.id) : []);
  useEffect(() => { if (!isDemo) void loadMaintenanceItems(vehicle.ownerUserId, vehicle.id).then(setMaintenanceItems); }, [vehicle.id, vehicle.ownerUserId, isDemo]);
  const closest = nextMaintenance(maintenanceItems, vehicle);
  const [search, setSearch] = useState(""); const [debouncedSearch, setDebouncedSearch] = useState("");
  useEffect(() => { const timer = window.setTimeout(() => setDebouncedSearch(search), 200); return () => window.clearTimeout(timer); }, [search]);
  const matches = useMemo(() => filterServiceEntries(sorted, debouncedSearch), [sorted, debouncedSearch]);
  return <main className="page vehicle-page"><button className="back-link" onClick={() => navigate({ name: "garage" })}>← My Garage</button><section className="vehicle-hero"><div><p className="kicker">{vehicle.nickname || "Vehicle record"}</p><h1>{vehicle.year} {vehicle.make}<br /><em>{vehicle.model}</em><span>.</span></h1><p>{vehicle.trim}</p></div><div className="hero-mileage"><span>Latest recorded mileage</span><strong>{vehicle.latestMileage?.toLocaleString() ?? "—"}</strong><small>{vehicle.latestMileage ? formatDate(vehicle.latestMileageDate) : "Add mileage with your next entry"}</small></div><div className="hero-actions"><button className="primary-button" onClick={() => navigate({ name: "addEntry", vehicleId: vehicle.id })}>＋ New entry</button><button className="quiet-button" onClick={onEdit}>Edit details</button><button className="quiet-button" onClick={onImport}>Import CSV</button><button className="quiet-button" onClick={onArchive}>Archive vehicle</button></div></section><section className="vehicle-summary"><div className="side-card reminder-card"><p className="kicker">Due & upcoming</p><h3>{closest?.name || "Nothing scheduled"}</h3><p>{closest ? dueText(closest, vehicle).replace(`${closest.name} · `, "") : "Add an item to the maintenance plan below."}</p></div><div className="side-card"><p className="kicker">Recorded costs</p><strong>{formatCost(total) ?? "$0.00"}</strong><p>Across {sorted.length} recorded {sorted.length === 1 ? "entry" : "entries"}</p></div></section><MaintenancePanel vehicle={vehicle} entries={sorted} items={maintenanceItems} setItems={setMaintenanceItems} /><VehicleReferencePanel vehicle={vehicle} /><section className="timeline-panel service-history"><div className="panel-heading"><div><p className="kicker">Service history</p><h2>The record</h2></div>{sorted.length > 0 && <span>{sorted.length} {sorted.length === 1 ? "entry" : "entries"}</span>}</div><label className="history-search"><span className="sr-only">Search service history</span><input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search service history…" /></label>{sorted.length === 0 ? <div className="empty-timeline"><h3>Record what happened</h3><p>Services, repairs, and observations will appear here.</p><button className="primary-button" onClick={() => navigate({ name: "addEntry", vehicleId: vehicle.id })}>Create first entry</button></div> : matches.length === 0 ? <p className="history-empty">No service entries match “{debouncedSearch}”.</p> : <div className="timeline">{matches.map((entry) => <TimelineItem entry={entry} key={entry.id} onEdit={() => navigate({ name: "editEntry", vehicleId: vehicle.id, entryId: entry.id })} />)}</div>}</section></main>;
}

function MaintenancePanel({ vehicle, entries, items, setItems }: { vehicle: Vehicle; entries: ServiceEntry[]; items: MaintenanceItem[]; setItems: Dispatch<SetStateAction<MaintenanceItem[]>> }) {
  const isDemo = vehicle.ownerUserId === "demo"; const [name, setName] = useState(""); const [interval, setInterval] = useState(""); const [unit, setUnit] = useState<"miles" | "months">("miles"); const [editing, setEditing] = useState<MaintenanceItem>();
  async function add() { if (!name.trim()) return; const values = { name: name.trim(), priority: "normal" as const, enabled: true, intervalMiles: unit === "miles" && interval ? Number(interval) : undefined, intervalMonths: unit === "months" && interval ? Number(interval) : undefined }; const id = isDemo ? `demo-maint-${Date.now()}` : (await addMaintenanceItem(vehicle.ownerUserId, vehicle.id, values)).id; setItems((current) => [...current, { id, ownerUserId: vehicle.ownerUserId, vehicleId: vehicle.id, ...values, schemaVersion: 1 }]); setName(""); setInterval(""); }
  async function remove(item: MaintenanceItem) { if (!confirm(`Remove ${item.name} from the maintenance plan?`)) return; if (!isDemo) await deleteMaintenanceItem(item.id); setItems((current) => current.filter((candidate) => candidate.id !== item.id)); }
  async function complete(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (!editing) return; const form = new FormData(event.currentTarget); const date = String(form.get("date")); const mileage = String(form.get("mileage")); const linked = String(form.get("linked")); const values = { lastDoneDate: date ? Timestamp.fromDate(new Date(`${date}T12:00:00`)) : undefined, lastDoneMileage: mileage ? Number(mileage) : undefined, linkedServiceEntryId: linked || undefined }; if (!isDemo) await updateMaintenanceItem(editing.id, values); setItems((current) => current.map((item) => item.id === editing.id ? { ...item, ...values } : item)); setEditing(undefined); }
  const active = items.filter((item) => item.enabled).sort((a, b) => compareMaintenanceDue(maintenanceValues(a), maintenanceValues(b), vehicle.latestMileage));
  return <section className="maintenance-panel"><div className="panel-heading"><div><p className="kicker">Maintenance plan</p><h2>Keep ahead</h2></div></div><div className="maintenance-add"><input aria-label="Maintenance item" value={name} onChange={(event) => setName(event.target.value)} placeholder="Oil change" /><input aria-label="Interval" value={interval} onChange={(event) => setInterval(event.target.value)} type="number" min="1" placeholder="Interval" /><select aria-label="Interval unit" value={unit} onChange={(event) => setUnit(event.target.value as "miles" | "months")}><option value="miles">Miles</option><option value="months">Months</option></select><button type="button" className="quiet-button" onClick={() => void add()}>Add item</button></div>{active.length === 0 ? <p className="field-help">Add an owner-defined item to start your maintenance plan.</p> : <div className="maintenance-table"><div className="maintenance-head"><span>Maintenance item</span><span>Interval</span><span>Last performed</span><span>Next due</span><span>Status</span><span>Actions</span></div>{active.map((item) => { const due = maintenanceDue(maintenanceValues(item), vehicle.latestMileage); const intervalText = [item.intervalMiles && `${item.intervalMiles.toLocaleString()} mi`, item.intervalMonths && `${item.intervalMonths} mo`].filter(Boolean).join(" / ") || "Not set"; const lastText = [item.lastDoneMileage != null && `${item.lastDoneMileage.toLocaleString()} mi`, item.lastDoneDate && formatDate(item.lastDoneDate)].filter(Boolean).join(" · ") || "Not recorded"; const nextText = [due.nextMileage != null && `${due.nextMileage.toLocaleString()} mi`, due.nextDate && due.nextDate.toLocaleDateString()].filter(Boolean).join(" · ") || "Not scheduled"; const label = due.status === "unscheduled" ? "Not scheduled" : due.status === "overdue" ? "Overdue" : "Upcoming"; return <div className="maintenance-row" key={item.id}><b data-label="Maintenance item">{item.name}</b><span data-label="Interval">{intervalText}</span><span data-label="Last performed">{lastText}</span><span data-label="Next due">{nextText}</span><span data-label="Status"><i className={`status-chip ${due.status}`}>{label}</i></span><span className="row-actions" data-label="Actions"><button type="button" onClick={() => setEditing(item)}>Mark complete</button><button className="remove-action" type="button" onClick={() => void remove(item)}>Remove</button></span></div>; })}</div>}{editing && <div className="modal-backdrop" role="presentation"><form className="completion-dialog" onSubmit={(event) => void complete(event)}><p className="kicker">{editing.name}</p><h3>Last performed</h3><label>Date<input name="date" type="date" defaultValue={new Date().toISOString().slice(0, 10)} /></label><label>Mileage<input name="mileage" type="number" min="0" defaultValue={vehicle.latestMileage} /></label><label>Link service entry <small>Optional</small><select name="linked" defaultValue={editing.linkedServiceEntryId ?? ""}><option value="">No linked entry</option>{entries.map((entry) => <option key={entry.id} value={entry.id}>{formatDate(entry.serviceDate)} — {entry.title || entry.description}</option>)}</select></label><div className="form-actions"><button type="button" className="quiet-button" onClick={() => setEditing(undefined)}>Cancel</button><button className="primary-button">Save completion</button></div></form></div>}</section>;
}

function VehicleReferencePanel({ vehicle }: { vehicle: Vehicle }) {
  const isDemo = vehicle.ownerUserId === "demo"; const [items, setItems] = useState<VehicleSpecification[]>([]); const [editing, setEditing] = useState<VehicleSpecification | "new">();
  useEffect(() => { if (!isDemo) void loadVehicleSpecifications(vehicle.ownerUserId, vehicle.id).then(setItems); }, [vehicle.id, vehicle.ownerUserId, isDemo]);
  async function save(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); const values = { group: String(form.get("group")) as VehicleSpecification["group"], label: String(form.get("label")).trim(), value: String(form.get("value")).trim(), unit: String(form.get("unit")).trim() || undefined, brand: String(form.get("brand")).trim() || undefined, source: String(form.get("source")).trim() || undefined, sourceUrl: String(form.get("sourceUrl")).trim() || undefined, notes: String(form.get("notes")).trim() || undefined, isVerified: form.get("verified") === "on", sortOrder: editing === "new" ? items.length : editing?.sortOrder ?? items.length }; if (editing === "new") { const id = isDemo ? `demo-spec-${Date.now()}` : (await addVehicleSpecification(vehicle.ownerUserId, vehicle.id, values)).id; setItems((current) => [...current, { id, ownerUserId: vehicle.ownerUserId, vehicleId: vehicle.id, ...values, schemaVersion: 1 }]); } else if (editing) { if (!isDemo) await updateVehicleSpecification(editing.id, values); setItems((current) => current.map((item) => item.id === editing.id ? { ...item, ...values } : item)); } setEditing(undefined); }
  async function remove(item: VehicleSpecification) { if (!confirm(`Delete ${item.label}?`)) return; if (!isDemo) await deleteVehicleSpecification(item.id); setItems((current) => current.filter((candidate) => candidate.id !== item.id)); }
  const groups: Array<[VehicleSpecification["group"], string]> = [["torque", "Torque"], ["fluid", "Fluid"], ["part", "Part"], ["general", "General"]];
  const current = editing === "new" ? undefined : editing;
  return <section className="reference-panel"><div className="panel-heading"><div><p className="kicker">Vehicle notes & reference</p><h2>Known facts</h2></div><button className="quiet-button" onClick={() => setEditing("new")}>＋ Add reference</button></div>{items.length === 0 ? <p className="field-help">Save torque specs, fluids, filters, part numbers, and useful notes for this vehicle.</p> : <div className="reference-groups">{groups.map(([key, label]) => items.some((item) => item.group === key) && <section key={key}><h3>{label}</h3><div className="reference-list">{items.filter((item) => item.group === key).map((item) => <article key={item.id}><div><b>{item.label}</b>{item.isVerified && <span className="verified-badge">Verified</span>}<p>{item.value}{item.unit ? ` ${item.unit}` : ""}</p>{item.brand && <small>{item.brand}</small>}{item.source && <small>Source: {item.source}{item.sourceUrl && <> · <a href={item.sourceUrl} target="_blank" rel="noreferrer">Open source</a></>}</small>}{item.notes && <small>{item.notes}</small>}</div><div className="row-actions"><button onClick={() => setEditing(item)}>Edit</button><button className="remove-action" onClick={() => void remove(item)}>Delete</button></div></article>)}</div></section>)}</div>}{editing && <div className="modal-backdrop"><form className="reference-dialog" onSubmit={(event) => void save(event)}><p className="kicker">Vehicle reference</p><h3>{editing === "new" ? "Add a fact" : "Edit fact"}</h3><div className="reference-form-grid"><label>Group<select name="group" defaultValue={current?.group ?? "torque"}>{groups.map(([key, label]) => <option value={key} key={key}>{label}</option>)}</select></label><label>Label<input name="label" required defaultValue={current?.label} placeholder="Wheel lug torque" /></label><label>Value<input name="value" required defaultValue={current?.value} placeholder="80" /></label><label>Unit <small>Optional</small><input name="unit" defaultValue={current?.unit} placeholder="lb-ft" /></label><label>Brand / part number <small>Optional</small><input name="brand" defaultValue={current?.brand} /></label><label>Source <small>Optional</small><input name="source" defaultValue={current?.source} placeholder="Owner's manual" /></label><label className="wide">Source page / URL <small>Optional</small><input name="sourceUrl" type="url" defaultValue={current?.sourceUrl} /></label><label className="wide">Notes <small>Optional</small><textarea name="notes" defaultValue={current?.notes} rows={3} /></label><label className="check-field"><input name="verified" type="checkbox" defaultChecked={current?.isVerified} /> Verified against the source</label></div><div className="form-actions"><button type="button" className="quiet-button" onClick={() => setEditing(undefined)}>Cancel</button><button className="primary-button">Save reference</button></div></form></div>}</section>;
}
...
function DocumentsPage({ ownerUserId, vehicles, entries, navigate, demo }: { ownerUserId: string; vehicles: Vehicle[]; entries: ServiceEntry[]; navigate: (view: View) => void; demo: boolean }) {
  const [attachments, setAttachments] = useState<Attachment[]>([]);
  const [loading, setLoading] = useState(!demo);
  useEffect(() => { if (demo) return; loadAttachments(ownerUserId).then(setAttachments).finally(() => setLoading(false)); }, [ownerUserId, demo]);
  return <main className="page documents-page"><div className="page-heading"><div><p className="kicker">Across your garage</p><h1>Documents<span>.</span></h1></div></div>{loading ? <p className="field-help">Loading documents…</p> : attachments.length === 0 ? <section className="empty-state compact-empty"><h2>No documents yet</h2><p>Add a receipt or photo to a service entry and it will appear here.</p></section> : <div className="document-grid">{attachments.map((attachment) => { const vehicle = vehicles.find((item) => item.id === attachment.vehicleId); const entry = entries.find((item) => item.id === attachment.ownerId); return <AttachmentCard key={attachment.id} attachment={attachment} vehicle={vehicle} entry={entry} onOpenEntry={attachment.ownerType === "service_entry" && vehicle ? () => navigate({ name: "editEntry", vehicleId: vehicle.id, entryId: attachment.ownerId }) : undefined} />; })}</div>}</main>;
}

function AttachmentCard({ attachment, vehicle, entry, onOpenEntry }: { attachment: Attachment; vehicle?: Vehicle; entry?: ServiceEntry; onOpenEntry?: () => void }) {
  return <article className="document-card">{attachment.downloadUrl && attachment.contentType.startsWith("image/") && <a href={attachment.downloadUrl} target="_blank" rel="noreferrer"><img src={attachment.downloadUrl} alt={attachment.fileName} /></a>}<div><p className="kicker">{attachment.documentType}</p><h3>{attachment.fileName}</h3><p>{vehicle ? vehicleName(vehicle) : "Vehicle document"}{entry ? ` · ${entry.title || entry.description}` : ""}</p><div className="document-actions">{attachment.downloadUrl && <a href={attachment.downloadUrl} target="_blank" rel="noreferrer">Open document</a>}{onOpenEntry && <button type="button" onClick={onOpenEntry}>View entry</button>}</div></div></article>;
}

function EntryAttachments({ entry, vehicle }: { entry: ServiceEntry; vehicle: Vehicle }) {
  const [attachments, setAttachments] = useState<Attachment[]>([]);
  useEffect(() => { if (vehicle.ownerUserId === "demo") return; loadAttachments(vehicle.ownerUserId).then((items) => setAttachments(items.filter((item) => item.ownerType === "service_entry" && item.ownerId === entry.id))); }, [entry.id, vehicle.ownerUserId]);
  return <section className="entry-documents"><p className="kicker">Documents</p><h2>Receipts & photos</h2>{attachments.length ? <div className="document-grid">{attachments.map((attachment) => <AttachmentCard key={attachment.id} attachment={attachment} vehicle={vehicle} entry={entry} />)}</div> : <p className="field-help">No documents are attached to this entry.</p>}</section>;
}
...
function EntryEditForm({ vehicle, entry, onCancel, onSave }: { vehicle: Vehicle; entry: ServiceEntry; onCancel: () => void; onSave: (values: EntryFormValues) => Promise<void> }) {
  const [saving, setSaving] = useState(false); const [error, setError] = useState(""); const [photoFiles, setPhotoFiles] = useState<File[]>([]);
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setSaving(true); const form = new FormData(event.currentTarget); try { await onSave({ description: String(form.get("description")).trim(), serviceDate: String(form.get("date")), mileage: String(form.get("mileage")).trim() ? Number(form.get("mileage")) : undefined, totalCostCents: normalizeMoneyToCents(String(form.get("cost"))), providerType: String(form.get("provider")) as ServiceEntry["providerType"] || undefined, photoFiles }); } catch { setError("We couldn't update this entry or upload its documents."); setSaving(false); } }
  return <main className="entry-page"><button className="back-link" onClick={onCancel}>← {vehicleName(vehicle)}</button><div className="entry-heading"><p className="kicker">Entry detail</p><h1>Update the record<span>.</span></h1></div><form className="entry-form" onSubmit={submit}><label className="description-field"><textarea name="description" required defaultValue={entry.description} rows={4} /></label><div className="entry-fields"><label>Date<input name="date" type="date" required defaultValue={entry.serviceDate.toDate().toISOString().slice(0, 10)} /></label><label>Mileage<input name="mileage" type="number" min="0" defaultValue={entry.mileage} /></label><label>Cost<div className="money-field"><span>$</span><input name="cost" type="number" min="0" step=".01" defaultValue={entry.totalCostCents == null ? "" : (entry.totalCostCents / 100).toFixed(2)} /></div></label><label>Performed by<select name="provider" defaultValue={entry.providerType ?? "diy"}><option value="diy">Me</option><option value="shop">Shop</option><option value="dealer">Dealer</option><option value="other">Other</option></select></label></div><label className="attachment-button"><span>+</span><b>Add more receipts or photos</b><small>{photoFiles.length ? `${photoFiles.length} ready to upload` : "JPEG, PNG, HEIC, or WebP · up to 10 MB"}</small><input className="sr-only" type="file" accept="image/*" multiple onChange={(event) => setPhotoFiles(Array.from(event.target.files ?? []))} /></label>{error && <p className="form-error">{error}</p>}<div className="entry-save"><button className="primary-button" disabled={saving}>{saving ? "Saving…" : "Save changes →"}</button></div></form><EntryAttachments entry={entry} vehicle={vehicle} /></main>;
}
