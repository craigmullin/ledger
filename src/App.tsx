...
import { addServiceEntry, addVehicle, archiveVehicle, importServiceEntries, loadEntries, loadVehicles, updateVehicle, uploadVehiclePhoto, type ImportedServiceEntryValues } from "./data";
      ...
      <div className="app-main">
        ...
        {loading ? <LoadingCards /> : (
          <>
            ...
            {view.name === "editVehicle" && selectedVehicle && <VehicleDetailsForm vehicle={selectedVehicle} onCancel={() => navigate({ name: "vehicle", vehicleId: selectedVehicle.id })} onSave={(values) => saveVehicleDetails(selectedVehicle.id, values)} onUpload={(file) => saveVehiclePhoto(selectedVehicle.id, file)} />}
...
function VehicleDetailsForm({ vehicle, onCancel, onSave, onUpload }: { vehicle: Vehicle; onCancel: () => void; onSave: (values: VehicleFormValues) => Promise<void>; onUpload: (file: File) => Promise<void> }) {
  const [saving, setSaving] = useState(false);
